...
class MenuComponent {
    ...
    private _getDropdownPopperConfig = (item: HTMLElement) => {
        ...
        const strategy: 'absolute' | 'fixed' | undefined =
            this._getItemOption(item, 'overflow') === true
                ? 'absolute'
                : 'fixed';
    ...
    private _showDropdown = (item: HTMLElement) => {
        if (
            EventHandlerUtil.trigger(this.element, 'kt.menu.dropdown.show') ===
            false
        ) {
            return;
        }
    ...
    private _hideDropdown = (item: HTMLElement) => {
        if (
            EventHandlerUtil.trigger(this.element, 'kt.menu.dropdown.hide') ===
            false
        ) {
            return;
        }
    ...
    private _showAccordion = (item: HTMLElement) => {
        if (
            EventHandlerUtil.trigger(this.element, 'kt.menu.accordion.show') ===
            false
        ) {
            return;
        }
    ...
    private _hideAccordion = (item: HTMLElement) => {
        if (
            EventHandlerUtil.trigger(this.element, 'kt.menu.accordion.hide') ===
            false
        ) {
            return;
        }
        ...
        if (item !== null && itemSubType === 'dropdown') {
            ...
            if (items.length > 0) {
                for (let i = 0, len = items.length; i < len; i++) {
                    ...
                    if (
                        items[i] !== null &&
                        this._getItemSubType(items[i] as HTMLElement) ===
                            'dropdown'
                    ) {
    ...
    private _link = (element: HTMLElement, e: Event) => {
        if (
            EventHandlerUtil.trigger(this.element, 'kt.menu.link.click') ===
            false
        ) {
            return;
        }
    ...
    public static createInstances = (selector: string) => {
        ...
        document.querySelectorAll(selector).forEach((el) => {
            ...
            if (!menuInstance) {
                menuInstance = new MenuComponent(
                    el as HTMLElement,
                    defaultMenuOptions
                );
            }
        ...
        document.addEventListener('click', (e) => {
            ...
            if (menuItems && menuItems.length > 0) {
                for (let i = 0; i < menuItems.length; i++) {
                    ...
                    const menuObj = MenuComponent.getInstance(
                        item
                    ) as MenuComponent;
                    if (
                        menuObj &&
                        menuObj.getItemSubType(item) === 'dropdown'
                    ) {
                        // eslint-disable-next-line @typescript-eslint/no-unused-vars
                        const menu = menuObj.getElement();
                        const sub = menuObj.getItemSubElement(
                            item
                        ) as HTMLElement;
                        if (
                            item === e.target ||
                        ...
                        if (
                            sub &&
                            (sub === e.target ||
                                sub.contains(e.target as HTMLElement))
                        ) {
                            continue;
                        }
                        menuObj.hide(item);
        ...
        window.addEventListener('resize', () => {
            ...
            throttle(
                ...
                () => {
                    ...
                    elements.forEach((el) => {
                        const menu = MenuComponent.getInstance(
                            el as HTMLElement
                        );
                        if (menu) {
                            menu.update();
