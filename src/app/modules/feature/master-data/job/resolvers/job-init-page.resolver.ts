...
export class JobTitleInitPageResolver extends BaseInitPageResolver {

    private masterDataManagementService = inject(MasterDataManagementService);

    resolve(route: ActivatedRouteSnapshot, state: RouterStateSnapshot) {
        return this.masterDataManagementService.jobTitleGetJobTitleInitPagePost({}).pipe(
            map((body) => {
                return <InitPageData>{
                    ...this.defaultInitPageData,

                    dC_ExamTypeList: (body?.dC_ExamTypeList || [])
                        .map((dto) => ({ dto: dto, value: dto.dC_ExamTypeId!, name: dto.name! })),
