...
type Full_Model = NonNullable<ServicePackageXService["service"]>;

@UntilDestroy()
@Component({
    selector: "app-service-package-service-single-selection-list-field",
    templateUrl: "./service-package-service-single-selection-list-field.component.html",
    styleUrls: ["./service-package-service-single-selection-list-field.component.scss"],
    changeDetection: ChangeDetectionStrategy.OnPush,
    // Must set both providers & viewProviders fot @Host to work
    providers: SkipSettingValueAccessorProviders,
    viewProviders: SkipSettingValueAccessorProviders,
})
export class ServicePackageServiceSingleSelectionListFieldComponent extends BaseSingleSelectionListFieldComponent<Full_Model> {
    ...
    public getTableData$ = () => {
        ...
        return combineLatest([
            ...
            map(([allUnselectableServices, allServices]) =>
                allServices.filter(s =>
                    !allUnselectableServices.some((us) => us.serviceId === s.serviceId)
                )),
            shareReplay(1),
        );
    }
