...
export class ScheduleTemplatesInitPageResolver extends BaseInitPageResolver {

    private resourceManagementService = inject(ResourceManagementService);

    resolve(route: ActivatedRouteSnapshot, state: RouterStateSnapshot) {
        return this.resourceManagementService.initPageGetScheduleTemplateInitPagePost({}).pipe(
            map((body) => {
                return <InitPageData>{
